...
const i18n = {
  ja: {
    ...
    introDesc: 'うーこの部屋ではアカウント登録せずに全ての機能を利用することができます。ただし原神おみくじなど一部ページでは情報を端末に保存しており、機種変更や別端末で開いた場合、異なるデータとして扱われます。当画面で事前にIDとパスワードを設定しておくことで、データの引継ぎが可能となります。',
    uidTitle: '現在のUID',
  ...
  en: {
    ...
    introDesc: 'You can use every feature of Uko\'s Room without creating an account. That said, some pages — like Genshin Omikuji — store your data on the device itself, so switching devices or opening the site on a different device will look like a different, blank account. Setting an ID and password here in advance lets you carry that data over.',
